...
|---|---|
| 캡처 단축키 | `Win+Shift+F7` |
| GUI 붙여넣기 | `Ctrl+V` (기본 붙여넣기 — 이미지) |
| CLI 붙여넣기 | `Ctrl+Alt+V` (경로 붙여넣기) |
| 캡처 도구 | [PickPick](https://picpick.app/) (Shift+F7 단축키 설정 필요) |

캡처 후 **이미지를 클립보드에 유지**하면서 경로는 내부적으로 저장합니다.

- **GUI (Claude Code 패널):** 그냥 `Ctrl+V` → 이미지 첨부
- **CLI (통합 터미널):** `Ctrl+Alt+V` → 파일 경로 붙여넣기

> **배경:** Claude Code CLI는 클립보드 이미지를 붙여넣을 수 없고 파일 경로만 허용합니다. GUI(확장프로그램)는 이미지/경로 모두 지원합니다. 이 스크립트는 하나의 캡처로 양쪽을 모두 해결합니다.

> **PickPick 설정:** PickPick 단축키를 `Shift+F7`로 지정하고 자동저장을 활성화하세요.
