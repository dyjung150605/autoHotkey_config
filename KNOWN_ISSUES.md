...
> **해결:** 이미지 소스를 `%TEMP%` 재저장 → **PicPick 자동저장 폴더(`D:\_captures`)의 최신 PNG**로 전환.
> `Ctrl+Alt+V` 시점에 최신 파일을 해결하고 `FileExist`로 검증, 무엇을 붙였는지 툴팁 표시.
...
- [x] stale 차단 — 보관 변수 대신 **붙임 시점에 최신 파일을 해결**(항상 최신, stale 변수 자체가 없음)
